...
router.post('/register', async (req, res) => {
    ...
    if (findUser) {
        ...
        try {
            ...
            res.json({ token: token, message: "Register Success", status: true, userId: user._id })
        } catch (error) {
...
router.post("/login", async (req, res) => {
    try {
        ...
        if (!user) {
            return res.json({ token: null, message: "User Not Found", status: false })
        }
        else {
            const validPassword = await bcrypt.compare(req.body.password, user.password)
            if (!validPassword) {
                return res.json({ token: null, message: "Not Correct User Credentials", status: false })
            }
            const data = {
                user: { id: user._id, }
            }
